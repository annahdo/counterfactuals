...
class realNVP(GenerativeModel):
    def __init__(self, prior, hps, data_info):
        ...
        Args:
            data_info: information of dataset to be modeled.
    ...
    def g(self, z):
        ...
        if self.data_info["data_set"] in ['ImageNet32', 'ImageNet64', 'CelebA']:
            x, x_off_2 = self.factor_out(x, self.order_matrix_2)
            x, x_off_3 = self.factor_out(x, self.order_matrix_3)

            if self.data_info["data_set"] in ['ImageNet64', 'CelebA']:
    ...
    def sample(self, num_samples=10, device="cpu", seed=0, std=None, fill_with=None, mean=None):
        ...
        C = self.data_info["data_shape"][0]
        H, W = self.data_info["data_shape"][1], self.data_info["data_shape"][2]
        torch.random.manual_seed((seed))
        z_sample = self.prior.sample((num_samples, C, H, W)).to(device) * std
        if mean is not None:
            z_sample = z_sample + mean
        if fill_with:
            z_sample = torch.ones_like(z_sample) * fill_with
    ...
    def forward(self, x, preprocess_noise=True):
        ...
        log_det_sum = 0
        z_outs, log_det = self.f(x)
        log_det_sum += torch.sum(log_det, dim=(1, 2, 3)) + log_det_pre
        log_p = torch.sum(self.prior.log_prob(z_outs), dim=(1, 2, 3))
    ...
    def loss(self, log_prob, log_det):
        ...
        loss = loss + self.hps.scale_reg * weight_scale
    ...
    def mask_grad(self, z, grad_mask=None):
        ...
        if grad_mask > 0:
            x, x_off_1 = self.factor_out(torch.zeros_like(z), self.order_matrix_1)
            x_off_1 = torch.zeros_like(x_off_1)
            x = torch.ones_like(x)
            ...
            if self.data_info["data_set"] in ['ImageNet32', 'ImageNet64', 'CelebA']:
                if grad_mask > 1:
                    ...
                    if grad_mask > 5:
                        x, x_off_3 = self.factor_out(x, self.order_matrix_3)
                        x_off_3 *= 0.0
                        # if self.data_info["data_set"] in ['ImageNet64', 'CelebA'] and grad_mask > 4:
                        #     x, x_off_4 = self.factor_out(x, self.order_matrix_4)
                        #     x_off_4 *= 0.0
                        #     if grad_mask > 5:
                        #         [B, _, _, _] = list(x.size())
                        #         x = x * (1. - self.s5_ckbd[-1].coupling.mask.repeat(B, 1, 1, 1))
                        #     x = self.restore(x, x_off_4, self.order_matrix_4)
                        #     if grad_mask > 6:
                        #         # SCALE 4: 8 x 8
                        #         x = self.squeeze(x)
                        #         x = self.channel_wise_mask(self, x, self.s4_chan[-1])
                        #         x = self.undo_squeeze(x)
                        #
                        #     for i in reversed(range(len(self.s4_ckbd))):
                        #         x, _ = self.s4_ckbd[i](x, reverse=True)
                    ...
                    if grad_mask > 4:
                        x = self.squeeze(x)
                        x = self.channel_wise_mask(self, x, self.s3_chan[-1])
                        x = self.undo_squeeze(x)

                        if grad_mask > 5:
                            [B, _, _, _] = list(x.size())
                            x = x * (1. - self.s3_ckbd[-1].coupling.mask.repeat(B, 1, 1, 1))
                    ...
                    if grad_mask > 2:
                        x = self.squeeze(x)
                        x = self.channel_wise_mask(self, x, self.s2_chan[-1])
                        x = self.undo_squeeze(x)

                        if grad_mask > 3:
                            [B, _, _, _] = list(x.size())
                            x = x * (1. - self.s2_ckbd[-1].coupling.mask.repeat(B, 1, 1, 1))

                mask = self.restore(x, x_off_1, self.order_matrix_1)
                ...
                if grad_mask > 2:
                    ...
                    mask = self.channel_wise_mask(self, mask, self.s1_chan[-1])
                    mask = self.undo_squeeze(mask)
    ...
    def channel_wise_mask(self, mask, s_chan):
        ...
        if s_chan.coupling.mask_config:
            (on, off) = mask.split(C // 2, dim=1)
        else:
            (off, on) = mask.split(C // 2, dim=1)
        off *= 0.0

        if s_chan.coupling.mask_config:
            mask = torch.cat((on, off), dim=1)
        else:
            mask = torch.cat((off, on), dim=1)
