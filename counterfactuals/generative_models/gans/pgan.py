...
class Generator(nn.Module):
    def __init__(self, n_label=1, nz=511):
        super().__init__()
    ...
    def forward(self, input, label=0, step=4, alpha=1):
        input = self.code_norm(input)
        batch_size = input.shape[0]

        label = self.label_embed(torch.zeros(batch_size).long().to(input.device))
        ...
        # rescale as model was trained with mean=std=[0.5, 0.5, 0.5])
        out = out / 2.
        out = out + 0.5
...
class pGAN(GenerativeModel):
    def __init__(self, n_label=1, nz=511, data_info=None, find_z=None):
        ...
        self.generator = Generator(n_label=n_label, nz=nz)
    ...
    def sample(self, batchsize):
        return self.generator.sample(batchsize)
